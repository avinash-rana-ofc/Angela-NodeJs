...
app.get("/filter", (req, res) => {
  ...
  const randomNum = Math.floor(Math.random() * filteredJokes.length);
  res.json(filteredJokes[randomNum]);
})

//4. POST a new joke
app.post("/jokes", (req, res) => {
  const id = jokes.length + 1;
  const joke = req.body.jokeText;
  const type = req.body.jokeType;
  
  jokes.push({id, jokeText : joke, jokeType : type});
